...
float3 linear_eval_irradiance(float4 sh_r, float4 sh_g, float4 sh_b, float3 v)
{
    float4 sh_dir = dir_to_sh(v);
    sh_dir = dir_to_irradiance_probe(sh_dir);//cosine conv
    return float3(dot(sh_r, sh_dir), dot(sh_g, sh_dir), dot(sh_b, sh_dir));
}

float3 hallucinate_zh3_irradiance(float4 sh_r, float4 sh_g, float4 sh_b, float3 v)
{
    ...
    float invzonalaxislen = rsqrt(dot(zonal_axis, zonal_axis));
    ...
    //cosine conv
    return linear_eval_irradiance(sh_r, sh_g, sh_b, v) + 0.25 * zonal_l2_coeff * zh_dir;
}
